...
public:
  ...
  createConfigFromProto(envoy::service::tap::v2alpha::TapConfig&& proto_config,
    ...
    return std::make_shared<SocketTapConfigImpl>(std::move(proto_config), context_.runtime(),
                                                 admin_streamer, context_.dispatcher().timeSource(),
                                                 context_.clusterManager(), context_.statsScope(),
                                                 context_.localInfo()
...
Network::TransportSocketFactoryPtr UpstreamTapSocketConfigFactory::createTransportSocketFactory(
  ...
  return std::make_unique<TapSocketFactory>(outer_config,
                                            std::make_unique<SocketTapConfigFactoryImpl>(context),
                                            context, std::move(inner_transport_factory));
}
