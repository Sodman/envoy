...
// we might not even need manager................. as we don't have more than one name per tap
filter!
  //
...
ExtensionConfigBase::ExtensionConfigBase(
    ...
    Init::Manager* init_manager, const std::string& stat_prefix, Stats::Scope& stats,
  ...
  case envoy::config::common::tap::v2alpha::CommonExtensionConfig::kTapdsConfig: {
    tap_config_provider_manager_ = singleton_manager.getTyped<TapConfigProviderManager>(
    ...
    subscription_ = tap_config_provider_manager_->subscribeTap(
        proto_config_.tapds_config(), *this, stat_prefix, stats, cluster_Manager, local_info,
        main_thread_dispatcher, random, api, validation_visitor);
    break;
  }
  default: {
...
void ExtensionConfigBase::newTapConfig(envoy::service::tap::v2alpha::TapConfig&& proto_config,
                                       Sink* admin_streamer) {
  tls_slot_->runOnAllThreads([this, proto_config, admin_streamer] {
