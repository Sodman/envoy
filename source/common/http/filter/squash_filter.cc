...
FilterHeadersStatus
SquashFilter::decodeHeaders(HeaderMap &headers, bool) {
  ...
  request->body().reset(
      new Buffer::OwnedImpl(config_->attachment_json()));
...
void SquashFilter::onSuccess(MessagePtr &&m) {
  ...
  case CREATE_CONFIG: {
    ...
    if (m->headers().Status()->value() != "201") {
      ...
      try {
        Json::ObjectSharedPtr json_config =
            Json::Factory::loadFromString(jsonbody);
        debugConfigId =
...
FilterDataStatus
SquashFilter::decodeData(Buffer::Instance &, bool) {
  if (state_ == INITIAL) {
...
SquashFilter::decodeTrailers(HeaderMap &) {
  if (state_ == INITIAL) {
...
const LowerCaseString &SquashFilter::squashHeaderKey() {
  static LowerCaseString *key =
      new LowerCaseString("x-squash-debug");
  return *key;
}
